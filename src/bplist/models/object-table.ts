...
export class ObjectTable extends BaseReader {
  ...
  constructor(buffer: ArrayBuffer, trailer: Trailer, version: string, logger: ILogger) {
    ...
    if (currentOffset !== offsetTableOffset) {
      this.logger.warn('End of object table should be equal to offsetTableOffset', { currentOffset, offsetTableOffset });
    }
  }
  ...
  parseObjectTableEntry(
    ...
    this.logger.debug('DBG: offset=%s found marker=%s with lowerNibble=0x%s', offset, Marker[marker], lowerNibble.toString(16))
    ...
    switch (marker) {
      ...
      case Marker.date: {
        ...
        const bytes = 2 ** lowerNibble;
        if (bytes !== 8) {
          this.logger.warn('Non-spec date; should be 8-bytes but is %d as lowerNibble is %s; that is fine though', bytes, lowerNibble.toString(16));
        }
    ...
    this.logger.debug('DBG: offset=%d done, read %d bytes and found %O', offset, bytesRead, entry);
